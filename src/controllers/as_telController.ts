...
import { as_telService } from "@root/services";
...
const as_get = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    let asenttel: Partial<Record<keyof ASENTTEL, string | number>> = {};

    const allowedFields = new Set<keyof ASENTTEL>(['TELNID_TEL','TELNID_ENT']);

    for (const [key, value] of Object.entries(req.query)) {
        const lowerKey = key.toUpperCase() as keyof ASENTTEL;

        if (allowedFields.has(lowerKey) && typeof value === 'string') {
            const originalKey = lowerKey.toUpperCase() as keyof ASENTTEL;
            asenttel[originalKey] = !isNaN(Number(value)) ? Number(value) : value;
        }
    }

    if (page && !isNaN(Number(page))) {
        asenttel.TELNPAGNUM = Number(page);
    }
    if (limit && !isNaN(Number(limit))) {
        asenttel.TELNPAGLIM = Number(limit);
    }

    const result = await as_telService.as_get(token as string, asenttel as ASENTTEL);
...
const as_create = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const asenttel = req.body as ASENTTEL;
    ...
    if (!asenttel) {
        return res.status(StatusCodes.BAD_REQUEST).json(new as_ErrorResponse("the Phone number data was not provided",StatusCodes.BAD_REQUEST));
    }

    await as_telService.as_Create(token, asenttel);
    
    return res.status(StatusCodes.CREATED).json({ message: 'Phone number created successfully' });
});
...
const as_update = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const allowedWhereFields = new Set<keyof ASENTTEL>(['TELNID_TEL','TELNID_ENT']);

    const whereConditions: Partial<Record<keyof ASENTTEL, string | number>> = {};
    
    for (const [key, value] of Object.entries(req.query)) {
        const originalKey = key.toUpperCase() as keyof ASENTTEL;
    ...
    const updateData: Partial<Record<keyof ASENTTEL, string | number>> = {};
    ...
    await as_telService.as_Update(token, updateData as Partial<ASENTTEL>, whereConditions as Partial<ASENTTEL>);

    return res.status(StatusCodes.OK).json({ message: 'Phone number updated successfully' });
});
...
const as_delete = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const [key, value] of Object.entries(req.query)) {
        const upperKey = key.toUpperCase() as keyof ASENTTEL;
        
        if (allowedWhereFields.has(upperKey) && (typeof value === 'string' || typeof value === 'number')) {
            asenttel[upperKey] = !isNaN(Number(value)) ? Number(value) : value;
        }
    }

    if (Object.keys(asenttel).length === 0) {
        return res.status(StatusCodes.BAD_REQUEST).json(new as_ErrorResponse(`At least one of the {${Array.from(allowedWhereFields).join(", ")}} valid parameters must be provided in parameters`, StatusCodes.BAD_REQUEST));
    }

    await as_telService.as_Delete(token as string, asenttel as ASENTTEL);

    return res.status(StatusCodes.OK).json({ message: 'Phone number deleted successfully' });
});

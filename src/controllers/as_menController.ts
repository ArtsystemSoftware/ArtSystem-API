...
import { as_menService } from "@root/services";
...
const as_get = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    let asentven: Partial<Record<keyof ASCADMEN, string | number>> = {};

    const allowedFields = new Set<keyof ASCADMEN>(['MENNID_MEN']);

    for (const [key, value] of Object.entries(req.query)) {
        const lowerKey = key.toUpperCase() as keyof ASCADMEN;

        if (allowedFields.has(lowerKey) && typeof value === 'string') {
            const originalKey = lowerKey.toUpperCase() as keyof ASCADMEN;
            asentven[originalKey] = !isNaN(Number(value)) ? Number(value) : value;
        }
    }

    if (page && !isNaN(Number(page))) {
        asentven.MENNPAGNUM = Number(page);
    }
    if (limit && !isNaN(Number(limit))) {
        asentven.MENNPAGLIM = Number(limit);
    }

    const result = await as_menService.as_get(token as string, asentven as ASCADMEN);
...
const as_create = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const asentven = req.body as ASCADMEN;
    ...
    if (!asentven) {
        return res.status(StatusCodes.BAD_REQUEST).json(new as_ErrorResponse("the Menu option data was not provided",StatusCodes.BAD_REQUEST));
    }

    await as_menService.as_Create(token, asentven);
    
    return res.status(StatusCodes.CREATED).json({ message: 'Menu option created successfully' });
});
...
const as_update = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const allowedWhereFields = new Set<keyof ASCADMEN>(['MENNID_MEN']);

    const whereConditions: Partial<Record<keyof ASCADMEN, string | number>> = {};
    
    for (const [key, value] of Object.entries(req.query)) {
        const originalKey = key.toUpperCase() as keyof ASCADMEN;
    ...
    const updateData: Partial<Record<keyof ASCADMEN, string | number>> = {};
    ...
    await as_menService.as_Update(token, updateData as Partial<ASCADMEN>, whereConditions as Partial<ASCADMEN>);

    return res.status(StatusCodes.OK).json({ message: 'Menu option updated successfully' });
});
...
const as_delete = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const [key, value] of Object.entries(req.query)) {
        const upperKey = key.toUpperCase() as keyof ASCADMEN;
    ...
    await as_menService.as_Delete(token as string, asentven as ASCADMEN);

    return res.status(StatusCodes.OK).json({ message: 'Menu option deleted successfully' });
});

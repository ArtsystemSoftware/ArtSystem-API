...
import { as_estService } from "@root/services";
...
const as_get = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    let asproest: Partial<Record<keyof ASPROEST, string | number>> = {};

    const allowedFields = new Set<keyof ASPROEST>(['ESTNID_EST','ESTNID_PRO','ESTNID_LOJ']);

    for (const [key, value] of Object.entries(req.query)) {
        const lowerKey = key.toUpperCase() as keyof ASPROEST;

        if (allowedFields.has(lowerKey) && typeof value === 'string') {
            const originalKey = lowerKey.toUpperCase() as keyof ASPROEST;
            asproest[originalKey] = !isNaN(Number(value)) ? Number(value) : value;
        }
    }

    if (page && !isNaN(Number(page))) {
        asproest.ESTNPAGNUM = Number(page);
    }
    if (limit && !isNaN(Number(limit))) {
        asproest.ESTNPAGLIM = Number(limit);
    }

    const result = await as_estService.as_get(token as string, asproest as ASPROEST);
...
const as_create = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const asproest = req.body as ASPROEST;
    ...
    if (!asproest) {
        return res.status(StatusCodes.BAD_REQUEST).json(new as_ErrorResponse("the Stock data was not provided",StatusCodes.BAD_REQUEST));
    }

    await as_estService.as_Create(token, asproest);
...
const as_update = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const allowedWhereFields = new Set<keyof ASPROEST>(['ESTNID_EST','ESTNID_PRO','ESTNID_LOJ']);

    const whereConditions: Partial<Record<keyof ASPROEST, string | number>> = {};
    
    for (const [key, value] of Object.entries(req.query)) {
        const originalKey = key.toUpperCase() as keyof ASPROEST;
    ...
    const updateData: Partial<Record<keyof ASPROEST, string | number>> = {};
    ...
    await as_estService.as_Update(token, updateData as Partial<ASPROEST>, whereConditions as Partial<ASPROEST>);
...
const as_delete = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const [key, value] of Object.entries(req.query)) {
        const upperKey = key.toUpperCase() as keyof ASPROEST;
        
        if (allowedWhereFields.has(upperKey) && (typeof value === 'string' || typeof value === 'number')) {
            asproest[upperKey] = !isNaN(Number(value)) ? Number(value) : value;
        }
    }

    if (Object.keys(asproest).length === 0) {
        return res.status(StatusCodes.BAD_REQUEST).json(new as_ErrorResponse(`At least one of the {${Array.from(allowedWhereFields).join(", ")}} valid parameters must be provided in parameters`, StatusCodes.BAD_REQUEST));
    }

    await as_estService.as_Delete(token as string, asproest as ASPROEST);

    return res.status(StatusCodes.OK).json({ message: 'Stock deleted successfully' });
});

...
import { as_ptpService } from "@root/services";
...
const as_get = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    let asprotab: Partial<Record<keyof ASPROTAB, string | number>> = {};

    const allowedFields = new Set<keyof ASPROTAB>(['PTPNID_PTP','PTPNID_PRO']);

    for (const [key, value] of Object.entries(req.query)) {
        const lowerKey = key.toUpperCase() as keyof ASPROTAB;

        if (allowedFields.has(lowerKey) && typeof value === 'string') {
            const originalKey = lowerKey.toUpperCase() as keyof ASPROTAB;
            asprotab[originalKey] = !isNaN(Number(value)) ? Number(value) : value;
        }
    }

    if (page && !isNaN(Number(page))) {
        asprotab.PTPNPAGNUM = Number(page);
    }
    if (limit && !isNaN(Number(limit))) {
        asprotab.PTPNPAGLIM = Number(limit);
    }

    const result = await as_ptpService.as_get(token as string, asprotab as ASPROTAB);
...
const as_create = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const asprotab = req.body as ASPROTAB;
    ...
    if (!asprotab) {
        return res.status(StatusCodes.BAD_REQUEST).json(new as_ErrorResponse("the table price data was not provided",StatusCodes.BAD_REQUEST));
    }

    await as_ptpService.as_Create(token, asprotab);
    
    return res.status(StatusCodes.CREATED).json({ message: `table price created successfully` });
});
...
const as_update = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const allowedWhereFields = new Set<keyof ASPROTAB>(['PTPNID_PTP','PTPNID_PRO']);

    const whereConditions: Partial<Record<keyof ASPROTAB, string | number>> = {};
    
    for (const [key, value] of Object.entries(req.query)) {
        const originalKey = key.toUpperCase() as keyof ASPROTAB;
    ...
    const updateData: Partial<Record<keyof ASPROTAB, string | number>> = {};
    ...
    await as_ptpService.as_Update(token, updateData as Partial<ASPROTAB>, whereConditions as Partial<ASPROTAB>);

    return res.status(StatusCodes.OK).json({ message: `table price updated successfully` });
});
...
const as_delete = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const [key, value] of Object.entries(req.query)) {
        const upperKey = key.toUpperCase() as keyof ASPROTAB;
        
        if (allowedWhereFields.has(upperKey) && (typeof value === 'string' || typeof value === 'number')) {
            asprotab[upperKey] = !isNaN(Number(value)) ? Number(value) : value;
        }
    }

    if (Object.keys(asprotab).length === 0) {
        return res.status(StatusCodes.BAD_REQUEST).json(new as_ErrorResponse(`At least one of the {${Array.from(allowedWhereFields).join(", ")}} valid parameters must be provided in parameters`, StatusCodes.BAD_REQUEST));
    }

    await as_ptpService.as_Delete(token as string, asprotab as ASPROTAB);

    return res.status(StatusCodes.OK).json({ message: `table price deleted successfully` });
});

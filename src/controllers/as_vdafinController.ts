...
import { as_vdafinService } from "@root/services";
...
const as_get = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    let asvalfin: Partial<Record<keyof ASFINFIN_DET, string | number>> = {};

    const allowedFields = new Set<keyof ASFINFIN_DET>(["FINCCODLOJ","FINNID_LOJ","FINDDATINI","FINDDATFIN",]);
    
    for (const [key, value] of Object.entries(req.query)) {
        const lowerKey = key.toUpperCase() as keyof ASFINFIN_DET;
        
        if (allowedFields.has(lowerKey) && typeof value === 'string') {
            const originalKey = lowerKey.toUpperCase() as keyof ASFINFIN_DET;
            asvalfin[originalKey] = !isNaN(Number(value)) ? Number(value) : value;
        }
    }

    const result = await as_vdafinService.as_get(token as string, asvalfin as ASFINFIN_DET);
...
const as_create = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const asvalfin = req.body as ASFINFIN_DET;
    ...
    if (!asvalfin) {
        return res.status(StatusCodes.BAD_REQUEST).json(new as_ErrorResponse("the finance data was not provided",StatusCodes.BAD_REQUEST));
    }

    await as_vdafinService.as_Create(token, asvalfin);
    
    return res.status(StatusCodes.CREATED).json({ message: 'finance created successfully' });
});
...
const as_update = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const allowedWhereFields = new Set<keyof ASFINFIN_DET>(['FINCCODLOJ','FINNID_LOJ']);

    const whereConditions: Partial<Record<keyof ASFINFIN_DET, string | number>> = {};
    
    for (const [key, value] of Object.entries(req.query)) {
        const originalKey = key.toUpperCase() as keyof ASFINFIN_DET;
    ...
    const updateData: Partial<Record<keyof ASFINFIN_DET, string | number>> = {};
    ...
    await as_vdafinService.as_Update(token, updateData as Partial<ASFINFIN_DET>, whereConditions as Partial<ASFINFIN_DET>);

    return res.status(StatusCodes.OK).json({ message: 'finance updated successfully' });
});
...
const as_delete = as_asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const [key, value] of Object.entries(req.query)) {
        const upperKey = key.toUpperCase() as keyof ASFINFIN_DET;
    ...
    await as_vdafinService.as_Delete(token as string, asvalfin as ASFINFIN_DET);

    return res.status(StatusCodes.OK).json({ message: 'finance deleted successfully' });
});
